...
<div class="">
    <div class="page-title">
      ...
      <div class="title_right">
        <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
              <a href="{{ URL::to('pendataan/sptpd/3') }}" class="btn btn-info"><i class="fa fa-plus"></i> Tambah Baru</a>
              <a href="{{ URL::to('pendataan/lihat_data_hibur') }}" class="btn btn-primary"><i class="fa fa-list"></i> Lihat Data</a>
        </div>
      </div>
    </div>
    ...
    @else
      @if(!$spr)
        <form action="{{url('pendataan/edit_data_hibur/'.$id_spt)}}" class="form-horizontal form-label-left" method="POST">
      @else
    ...
    <div class="row">
      ...
      <div class="col-md-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>History Pembayaran Pajak Hiburan</h2>
          ...
          <div class="x_panel">
            <div class="x_content">
                ...
                <div class="item form-group">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <textarea id="keterangan" class="form-control col-md-7 col-xs-12" type="text" name="keterangan">{{ (isset($id_spt)) ? $spt->spt_alamat_penerima : '' }}</textarea>
                  </div>
                </div>
                ...
                <div class="item form-group">
                  <table id="tbl_detail" class="table table-striped table-bordered">
                    ...
                    <input type='hidden' id="no" value='{{ (isset($id_spt) ? count($spt_detail) : 0)}}'>
                    @if(isset($id_spt))
                    <?php $x = 0;?>
                    @foreach($spt_detail as $dt)
                    <tr id="row{{$x}}">
                      <td><select class="form-control" id="korek.{{$x}}" name="korek[]" onchange="gantikorek(this.id,this.value)">
                        @foreach($korek as $k)
                          @if($k->korek_id == $dt->spt_dt_korek)
                          <option value="{{ $k->korek_id }};{{ $k->korek_persen_tarif}}" selected>({{ $k->korek_tipe.'.'.$k->korek_kelompok.'.'.$k->korek_jenis.'.'.$k->korek_objek.'.'.$k->korek_rincian.'.'.$k->korek_sub1}}) {{  $k->korek_nama}}</option>
                          @else
                      ...
                      <td><input type="number" class="form-control" id="dasar_pengenaan{{$x}}" name="dasar_pengenaan[]" onchange="gantipajak(this.id,this.value)" value="{{ $dt->spt_dt_jumlah }}"></td>
                      <td><input type="number" class="form-control" id="persen{{$x}}" name="persen[]" value="{{ $dt->spt_dt_persen_tarif }}"></td>
                      <td><input type="number" class="form-control" id="pajak{{$x}}" name="pajak[]" value="{{ $dt->spt_dt_pajak }}" readonly></td>
    ...
    var id_spt = '{{$id_spt or ''}}';
      ...
      function gethistory(wp_wr_id){
        ...
        oTable2 = $('#tbl_hist').DataTable({
          ...
          bFilter: false,   
          ajax: {
            url: "{{ URL::to('pendataan/gethistory/hibur') }}/"+wp_wr_id,
            type: "GET",
